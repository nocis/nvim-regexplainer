...
Set_Keys = function(win, buf)
  api.nvim_buf_set_keymap(
    ...
    'n',
    '<CR>',
    string.format([[:<C-U>lua require'regexplainer.localtest'.Open()<CR>]], win, buf),
  ...
  api.nvim_buf_set_keymap(
    ...
    'n',
    'q',
    ':lua require"regexplainer.localtest".Close()<CR>',
    { nowait = true, noremap = true, silent = true }
  )
end
Get_paths = function()
...
function M.highlight()
  ...
  for k, v in pairs(M.table_list) do
    if v ~= '' then
      local regex_cmd = string.format(
        "echo 'let res=[];patt=%sd; while (match = patt.exec(`%s`)) {res.push([match.index ,patt.lastIndex])} console.log(res);' | node -",
        M.regex,
        v
      )

      M.nodeJobId[k] = vim.fn.jobstart(regex_cmd, {
        on_stdout = function(_, data)
          for i, line in ipairs(data) do
            if line ~= '' then
              local linepos = line:gsub('%[', '{'):gsub('%]', '}')
              -- print(linepos)
              linepos = assert(load('return ' .. linepos))()
              -- print(linepos)
              for j, item in ipairs(linepos) do
                if item ~= '' then
                  -- print(item[1],item[2])
                  api.nvim_buf_add_highlight(M.list_buffer, 0, 'reg_hg', k - 1, item[1], item[2])
                  if M.nodeJobId[k] then
                    vim.fn.jobstop(M.nodeJobId[k])
                  end
                end
              end
            end
